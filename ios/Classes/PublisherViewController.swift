...
class PublisherViewController: NSObject, FlutterPlatformView {
    ...
    deinit {
        if self.methodCallHandler.loggingEnabled {
            print("[DEINIT] PublisherViewController")
        }
    }

    public func view() -> UIView {
        return methodCallHandler?.provider?.publisherView ?? openTokView
    }
}
